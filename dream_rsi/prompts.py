"""Prompt templates (paper Appendix B, made concrete for this runtime).

Two prompts matter:
  * exploration prompt  -> used by the discovery agent during online rollout
  * improvement prompt  -> used by the policy-development agent during dreaming
"""
from __future__ import annotations

from typing import Any, Dict, List, Optional

FILE_PROTOCOL = """## Output protocol (mandatory)

Emit every file you write with this exact framing, nothing else outside it:
...
- It is executed exactly as written, immediately, with no human in the loop. It must
  be complete, syntactically valid, and self-contained.
- No placeholders of any kind: no `...`, no `?`, no `TODO`, no pseudo-code, no
  "fill this in", no commented-out alternatives, no ellipsis in a literal.
- No commentary about what you might do inside the file. If you want to reason,
  write `proposal.md` and keep the program clean.
- Import only the standard library and numpy. No network, no file I/O, no
  printing on import, no long work at import time: the module must import fast and
  only do its work when the entry-point function is called.
...
## Discovery history so far (measured, trust the numbers)

{history_snapshot}

## Your branch and attempt

{branch_context}

## Files

Write only `{eval_program}` (the program that will be evaluated) and `proposal.md` (mechanism, evidence from history, why it's not a repeat, expected benefit/risk). Everything else is read-only.
...
POLICY_IMPROVEMENT_PROMPT = """You are improving one **prefix-only exploration policy**. Edit only
`{method_file}` and implement `OptimalPolicy.solve(self, question, budget=None)`.

## Objective: quality, work, and parallelism

The environment is a frozen, irregular branch x attempt grid. A policy opens a root
...
## API
    ...
    question.legal_actions() -> list[str]           # cell ids: roots + opened-branch frontiers
    question.legal_roots() -> list[str]             # unopened roots only
    question.opened_branches() -> list[int]
    question.meta(cell_id) -> CellMeta              # .cell .branch .attempt .parent_id .seq .tags
    question.probe_batch(cells, on_reveal=...) -> list[Observation]
    question.close_branch(branch_id)                # retire a branch for this episode
    question.baseline_score
    question.max_parallelism

`Observation` supplies `cell`, `branch`, `attempt`, `parent_id`, `score`,
`evaluated`, `valid`, `fail_class`, `error`, `delta_vs_baseline`,
`delta_vs_parent`, `n_valid`, `n_total`, `revealed_at_round`.
Helpers in `see.policy.api`: `branch_promising`, `branch_failed_hard`,
...
Do **not** use `question.best_so_far` or `question.probes_spent` to decide what
...
2. Rank legal roots and legal branch frontiers using only prefix-derived signals:
   ...
   versus failure evidence, failure recoverability, prior repair outcomes, remaining
   depth, and cross-branch comparison.
...
Minimal structure:
    ...
    from see.policy.api import (
        LLMDesignedMethod, SimResult, GridPlan, GridPlanningContext,
    ...
    class OptimalPolicy(LLMDesignedMethod):
        ...
        def _schedule(self, beta):
            ...  # one place for every threshold

        def plan_grid(self, context: GridPlanningContext) -> GridPlan:
            ...  # must return a GridPlan on every path

        def select_batch(self, question):
            prefix = question.observed()
            ...
            return batch  # list of legal cell ids, len <= question.max_parallelism

`select_batch` is called once per decision round by the runner (live and replay).
You may instead implement `solve(self, question, budget=None)` and call
`question.probe_batch` yourself; if you do, the base `select_batch` must not be
left unimplemented.
...
- Only these imports are available: the standard library, `see.policy.api`, and
  `dream_rsi.policy_api`. No network access, no file I/O.
...
1. **Within one replay or live episode:** beta is fixed. Route every behavioral
   ...
   `solve()`/`select_batch()`. Route recovery eligibility, reserve threshold, and waiting
   through the same schedule: high beta is more patient; low beta remains selective
   without treating one repairable failure as automatic closure.
...
Use the following cross-cycle default-beta rule. Read the most recent 2--3 **live**
`trace_pool/iter*/live_cycle_manifest.json` sidecars (and `_current` when present)
for each iteration's final best score and actual baked-in beta. Read the matching
archived `beta_sweep.json` values (`pareto.reward`, AUC, parallel penalty, and the
per-beta frontier). Scores alone do not establish that beta caused a change, so
...
`GridPlan(branch_count=W, refine_count=R, reason=...)` accepts arbitrary integers, not a
...
roots their directions, and `solve`/`select_batch` still decides which legal
roots/frontiers to open, refine, prune, or stop. Do not choose roots merely because
their branch id is small. The runtime grid is the hard bound: controller thresholds
may use less, but can never create branches or attempts beyond the effective plan.
Before finishing, verify that the edited `{method_file}` contains an override of `plan_grid` that
returns `GridPlan(branch_count=..., refine_count=..., reason=...)` on every path.

## Learn from history without leaking outcomes

Earlier cycles are in `{history_dir}`. Read their policy code and
...
stalls. The baseline under `{baseline_dir}` is a parallel-refine floor to beat.

Each current-objective cycle also archives
...
only**: never read it inside `solve()`/`select_batch()`, and never copy a trace-specific branch,
cell id, score, or target into policy logic.

## Replay feedback for this cycle

{feedback}

## Deliverable

Write a complete adaptive policy in `{method_file}`. Keep it compact: under ~250
lines, no restatement of these instructions, no duplicated helpers, no prose
outside the module docstring. A complete short policy beats a truncated long one. Include a short module
docstring describing its prefix signals, batch rule, beta schedule, default-beta
rationale, grid-planning rule, and safeguards against over-pruning, over-stopping,
permanent starvation after repairable failures, and serial probes. Before finishing,
verify trajectory-based ranking, the stated success semantics, non-automatic
zero-valid closure, deterministic recovery competition, and portfolio-level stop.

{file_protocol}"""


def exploration_prompt(task_desc: str, scoring_desc: str, eval_program: str,
                       node_dir: str, history_dir: str, baseline_dir: str,
                       problem_file: str, direction_guidance: str,
                       branch_context: str, history_snapshot: str = "",
                       ) -> str:
    return EXPLORATION_PROMPT.format(
        direction_guidance=direction_guidance or "(none)",
        node_dir=node_dir, history_dir=history_dir, baseline_dir=baseline_dir,
        eval_program=eval_program, problem_file=problem_file,
        task_description=task_desc, scoring_description=scoring_desc,
        history_snapshot=history_snapshot or "(no previous attempts yet)",
        branch_context=branch_context, file_protocol=FILE_PROTOCOL)


def improvement_prompt(method_file: str, history_dir: str, baseline_dir: str,
                       feedback: str) -> str:
    return POLICY_IMPROVEMENT_PROMPT.format(
        method_file=method_file, history_dir=history_dir,
        baseline_dir=baseline_dir, feedback=feedback,
